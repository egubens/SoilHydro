...
- fits the Van Genuchten model per treatment/group with bounded L‑BFGS‑B optimization;
...
**Requires**: R ≥ 3.6; Depends on base **stats** and **ggplot2**.

# Data requirements

Your long/tidy dataset should include at least:

- an **ID/treatment** column (optional for single‑profile fits), e.g. `Product`;
...
The package uses **Mualem's** linkage \( m = 1 - 1/n \) and fits parameters by minimizing the sum of squared errors (SSE) with **L‑BFGS‑B** bounds to enforce \( \theta_s>\theta_r\), \(n>1\), \( \alpha>0\).
...
We define a “near‑saturation” point at 10 kPa (field‑saturated water content proxy) and **PWP** at 1500 kPa. **AWC** is \( \theta_{sat} - \theta_{pwp} \).
...
- **Meso**: \( 10\text{–}1000\,\mu\text{m} \) (plant‑available water);
- **Micro**: \( d<10\,\mu\text{m} \) (tightly held water).

By default, **residual water** \( \theta_r \) is included within “micro” so that macro+meso+micro \(\approx \theta_s\).
...
`vg_fit_optim()` performs bounded least‑squares with sensible starting values per group. Returned diagnostics include **R²** and **RMSE** computed on the fitting data.
...
The 10 kPa “saturation” is a pragmatic proxy (near field capacity depends on texture/structure). PWP at 1500 kPa is conventional for many crops. **AWC** helps compare treatments but should be complemented by conductivity/retention hysteresis where available.
...
- **No fits converged**: verify units, remove non‑finite values, ensure \( \theta \in [0,1.2] \) (or realistic bounds).
- **R² is NaN**: occurs when all \( \theta \) values are identical; broaden suction range or re‑measure.
- **Unrealistic \( \alpha, n \)**: check data coverage; tighten bounds via `lower`/`upper` in `vg_fit_optim()`.

# API quick reference

- **Fitting/prediction**
  - `vg_fit_optim(data, id, theta, h, units)` → per‑group parameters + diagnostics.
  - `vg_predict(params_df, id_col, new_h, units, filter_id)` → \( \theta(h) \) table.

- **Derived metrics**
  - `vg_water_points(params_df, id_col, sat_kPa=10, pwp_kPa=1500)` → \( \theta_{sat}, \theta_{pwp}, \text{AWC} \).
  - `vg_pore_classes(params_df, id_col, d_micro_um=10, d_macro_um=1000, percent_basis=c("theta_s","available"), include_residual_in_micro=TRUE, return_residual=FALSE)` → macro/meso/micro volumes & %.

- **Plotting**
  - `plot_vg_fits(data, params_df, id, theta, h, units, log_x=TRUE)` → WRC plots by ID.
  - `plot_pore_classes_percent(psd_df, id_col, horiz=FALSE)` → stacked % bars.
  - `plot_pore_classes_volume(psd_df, id_col, horiz=FALSE)` → stacked volume bars.

# Reproducibility

For manuscripts, record R and package versions and set a seed for any simulation steps (as done above). Export figures with explicit sizes (e.g., `ggsave(..., width=8, height=5, units='cm', dpi=300)`) when preparing journal‑ready graphics.
